...
void vote_branch(pg_msht_t *h, marker_t *mk, vote_t *vt)
{
    ...
    if (cnt1 < NOISE_THRESHOLD) cnt1 = 0;
    if (cnt2 < NOISE_THRESHOLD) cnt2 = 0;
    if (cnt1 == 0 && cnt2 == 0) found = 0;
...
// hap: which haplotype path a result belongs to.
//   HAP_UNSPLIT (-1) -> produced before any haplotype split was found (homozygous walk)
//   0                -> the "left" haplotype path, taken at the first split
//   1                -> the "right" haplotype path, taken at the first split
void push_result(result_list_t *rl, int node_id, char side, int matches, int hap)
{
    ...
    rl->a[rl->n++] = (clust_result_t){node_id, side, matches, hap};
}

// split_done: 0 until the walk has forked into the two haplotype paths, 1 after.
// hap: HAP_UNSPLIT before the fork; 0 or 1 once committed to a path.
               ...
               int split_done, int hap)
{
    ...
    if (!split_done) {
        int tie = (vt.left_votes == vt.right_votes);
        int both_have_evidence = (vt.left_votes > 0 && vt.right_votes > 0);

        if (tie || both_have_evidence) {
            // First point where the two haplotypes diverge (or a genuine tie
            // with no evidence at all): fork into both branches. From here on
            // each branch is its own haplotype path and won't fork again.
            int lm = matches + vt.left_votes;
            int rm = matches + vt.right_votes;

            if (nd->left_child == 0) push_result(results, node_id, 'L', lm, 0);
            else walk_tree(h, t, nd->left_child, lm, results, 1, 0);

            if (nd->right_child == 0) push_result(results, node_id, 'R', rm, 1);
            else walk_tree(h, t, nd->right_child, rm, results, 1, 1);
            return;
        }
    }

    // Single-path mode: either we haven't split yet and only one side has any
    // evidence, or we already forked earlier and are now committed to one of
    // the two haplotype paths — either way, just take the best branch.
    if (vt.left_votes >= vt.right_votes) {
        int m2 = matches + vt.left_votes;
        if (nd->left_child == 0) push_result(results, node_id, 'L', m2, hap);
        else walk_tree(h, t, nd->left_child, m2, results, split_done, hap);
    } else {
        int m2 = matches + vt.right_votes;
        if (nd->right_child == 0) push_result(results, node_id, 'R', m2, hap);
        else walk_tree(h, t, nd->right_child, m2, results, split_done, hap);
    }
}

// Fills *hap1/*hap2 with the best result on each haplotype path and returns
// which of the four states was actually found:
//   CLUST_HOMOZYGOUS   - no fork happened, and a single result resolved;
//                        *hap1 is that result, *hap2 is the sentinel.
//   CLUST_HETEROZYGOUS - a fork happened and both sides resolved; *hap1 and
//                        *hap2 are each side's best result.
//   CLUST_AMBIGUOUS    - a fork happened but only one side produced a
//                        result. walk_tree's fork step always recurses into
//                        both children together, so this shouldn't occur —
//                        it's reported explicitly rather than silently
//                        collapsed into either of the other states, so
//                        callers can detect and log the anomaly. *hap1 gets
//                        whichever side did resolve; *hap2 is the sentinel.
//   CLUST_UNRESOLVED   - no fork happened AND no homozygous result was
//                        found either (e.g. an empty result list from a
//                        failed or empty walk). *hap1 and *hap2 are both
//                        the sentinel. Distinct from CLUST_HOMOZYGOUS so
//                        callers don't mistake "nothing to report" for a
//                        genuine single-haplotype call.
clust_zygosity_t pick_best2(result_list_t *rl, clust_result_t *hap1, clust_result_t *hap2)
{
    clust_result_t none = {0, '?', 0, HAP_UNSPLIT};
    clust_result_t best0 = none, best1 = none;
    int has0 = 0, has1 = 0;

    for (int i = 0; i < rl->n; ++i) {
        clust_result_t *r = &rl->a[i];
        if (r->hap == 0) {
            if (!has0 || r->matches > best0.matches) { best0 = *r; has0 = 1; }
        } else if (r->hap == 1) {
            if (!has1 || r->matches > best1.matches) { best1 = *r; has1 = 1; }
        }
    }
    ...
    if (has0 && has1) {
        *hap1 = best0;
        *hap2 = best1;
        return CLUST_HETEROZYGOUS;
    }

    if (has0 || has1) {
        *hap1 = has0 ? best0 : best1;
        *hap2 = none;
        return CLUST_AMBIGUOUS;
    }

    // Homozygous: nothing forked, pick the single best HAP_UNSPLIT result.
    clust_result_t best = none;
    int has = 0;
    for (int i = 0; i < rl->n; ++i) {
        if (rl->a[i].hap != HAP_UNSPLIT) continue;
        if (!has || rl->a[i].matches > best.matches) { best = rl->a[i]; has = 1; }
    }
    *hap2 = none;
    if (!has) {
        *hap1 = none;
        return CLUST_UNRESOLVED; // nothing forked, and nothing homozygous either -> empty/failed walk
    }
    *hap1 = best;
    return CLUST_HOMOZYGOUS;
}
...
chr_result_t *classify_cenhaps(pg_msht_t *h, chr_forest_t *forest, int *n_out)
{
    ...
    for (int c = 0; c < forest->n; ++c) {
        ...
        walk_tree(h, t, 1, 0, &results, 0, HAP_UNSPLIT);

        out[c].chr = forest->a[c].chr; // borrowed, freed with forest
        out[c].zygosity = pick_best2(&results, &out[c].hap1, &out[c].hap2);
